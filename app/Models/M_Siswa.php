...
class M_Siswa extends Model
{
    ...
    {
        return $this->db->table('data_kegiatan')->update($data, ['id' => $id]);
    }

    public function getDataById2($id)
    {
        $this->db->table('data_kegiatan')->where('id', $id);
        return $this->db->table('data_kegiatan')->get()->getRowArray();
    }
    public function detail_Data2($id_file)
    {
        return $this->db->table('data_kegiatan')->where('id', $id_file)->get()->getRowArray();
    }

    public function getAllData2()
    {
        return $this->db->table('data_kegiatan')->get()->getResultArray();
    }


    public function tambah2($data)
    {
        return $this->db->table('data_kegiatan')->insert($data);
    }
    public function hapus2($id, $file_peta_jalan, $file_sesuai_petajalan)
    {
        ...
        return $this->db->table('data_kegiatan')->delete(['id' => $id]);
    }
    ...
    {
        $sql = "SELECT COUNT(namamahasiswa)+COUNT(namadosen)/100 AS jumlah FROM `data_kegiatan` WHERE namamahasiswa != ''GROUP BY tahun ORDER by tahun";
    ...
    {
        $sql = "SELECT tahun FROM `data_kegiatan` GROUP BY tahun ORDER by tahun";
        $hasil = $this->db->query($sql);
        if ($hasil) {
            return $hasil->getResultArray();
        } else {
